...
\section{The cost of intitalization}

\section{Comparing blocking and non-blocking get operations}

A significant part for the initialization time involves fetching data from the manager. This takes
place at each time step. Our library support asynchronous get operations, which allow one to 
overlap communication with computation. 

The default mode to ``get'' data is blocking, that is retrieved data are ready when the ``get'' call
completes. Alternatively, the programmer may rely on ``getAsync'' calls. In this case the programmer 
indicates his/her desire to fetch the data but the data need not be immediately available. A ``startEpoch''
and ``endEpoch'' calls around the ``getAsync" operations determine the the time interval 
when remote memory access operations take place with 
``endEpoch'' indicating that the data must available at that point. (An additional ``flush'' can be used 
) to request completed ``get'' operations within a period. 

In Fig. \ref{fig:async} we compare the  execution time of MPI ``get'' operations using 
blocking and non-blocking approaches. Each test involves reading $N$ chunks ($=$ number of workers) 
...
of the big array. The benefit of using non-blocking ``get'' calls is particulalry apparent for large chunks 
